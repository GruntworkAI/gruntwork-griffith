...
| `dependencies.scan_status` | enum | `tier1_only` (current default — no CVE scan requested); `ok`, `sca_requested_and_failed`, `sca_requested_and_timed_out`, `sca_malformed_output` come from Unit 6. **Consumers MUST check `scan_status == "ok"` before treating zero CVEs as clean.** |
...
| `dependencies.sca` | object or null | Tier 2 CVE result. `null` when `--sca` is not used (current Unit 5 scope); populated in Unit 6. |
